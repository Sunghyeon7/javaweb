...
//		System.out.println("pdstest 에서 넘어온 파일명 : " + fName);
		
		
		
		/*
		 * COS라이브러리에서 제공해주는 객체 : MultipartRequest() / DefaultFileRenamePolicy()
		 * 사용법 : MultpartRequest(저장소명(request, "서버에 저장될 파일의 경로", "서버에 저장될 파일의 최대 용량", "코드변환방식", 기타 옵션(파일명 중복 방지 처리))
		 * 
		 * ServletContext application = request.getSurvletContext();
		 * String realPath = request.getServletContext().getRealPath("/");
		 * */
		
		String realPath = request.getServletContext().getRealPath("/images/pdstest");
//		System.out.println("realPath : " + realPath);
		int maxSize = 1024 * 1024 * 10;	// 서버에 저장할 최대용량은 10MByte
		String encoding = "UTF-8";
		
		// 파일 업로드 처리 (객체가 생성되면서 파일이 업로드 된다.)
		...
		System.out.println("서버에 저장된 경로 : " + realPath);
		System.out.println("서버에 저장된 파일명 : " + filesystemName);
		
		if(!originalFileName.equals("")) {
			request.setAttribute("msg", "파일 업로드 완료.");
		}
		else {
