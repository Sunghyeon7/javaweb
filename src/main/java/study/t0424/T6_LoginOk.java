...
//			String viewPage ="";
			...
			if((mid.equals("admin") && pwd.equals("1234")) || (mid.equals("hkd1234") && pwd.equals("1234"))) {
				...
				cookieMid.setPath("/");	// 웹 어플리케이션의 '전체(요청)경로'에서 사용하고자 할떄는 '/'로 지정한다.
				if(idSave.equals("on")) {
					cookieMid.setMaxAge(60*60*24*7); 		// 쿠키의 만료시간 7일
					response.addCookie(cookieMid);
				}
				else {
					cookieMid.setMaxAge(0); 		// 쿠키의 만료시간 7일				
				}
				response.addCookie(cookieMid);
				out.print("<script>");
				out.print("alert('"+mid+" 로그인 되었습니다.');");
				out.print("location.href='"+request.getContextPath() +"/study/0425_storage/t6_member.jsp';");
				out.print("</script>");
				
//				viewPage = "study/0425_Storage/t6_member.jsp";
...
//				viewPage = "study/0425_Storage/t6_Login.jsp";
//				request.getRequestDispatcher(viewPage).forward(request, response);
			}
