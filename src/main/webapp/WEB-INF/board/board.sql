...
select weekday(now());  /* 0 1 2 3 4 5 6 월~일 */
select dayofweek(now()) /* 1 2 3 4 5 6 7 일~토 */
...
/* 날짜양식(date_format()) : 년(%Y), 월(%m), 일(%d) */
select wDate, date_format(wDate, '%Y-%m-%d %H:%i') from board;
select *,date_format(wDate, '%Y-%m-%d'), timestampdiff(hour, wDate, now()) as hour_diff from board;
