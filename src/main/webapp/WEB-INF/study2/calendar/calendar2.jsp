...
<div class="container">
	<div class="text-center">
		...
		<button type="button" onclick="location.href='${ctp}/Calendar2.st?yy=${yy}&mm=${mm-1}';" class="btn btn-secondary btn-sm" title="이전 달">◀</button>
		<font size="5">${yy}년 ${mm+1}월</font>
		<button type="button" onclick="location.href='${ctp}/Calendar2.st?yy=${yy}&mm=${mm+1}';" class="btn btn-secondary btn-sm" title="다음년도">▶</button>
		<button type="button" onclick="location.href='${ctp}/Calendar2.st?yy=${yy+1}&mm=${mm}';" class="btn btn-secondary btn-sm" title="다음 달">▷▷</button>
		&nbsp; &nbsp;
		<button type="button" onclick="location.href='${ctp}/Calendar2.st';" class="btn btn-secondary btn-sm" title="현재 월">홈</button>
	</div>
	<br/>
	<div class="text-center">
...
<%-- 				<!-- 시작일 이전을 공백처리한다 (오늘이 월요일이면 startWeek가 2이기에 1칸을 공백처리한다. -->
				...
				<!-- 시작월 이전의 공백을 이전달의 날짜로 채운다. -->
				<c:set var="gap" value="${1}"/>
     		 	<c:forEach var="prevDay" begin="${preLastDay - (startWeek - 2)}" end="${preLastDay}" varStatus="st">
     		 		<td style="color:#ccc;font-size:0.8em">${prevMonth+1}-${prevDay}</td>
	  	    	<c:set var="gap" value="${gap + 1}"/>
      			</c:forEach>
				
				<!-- 해당월에 대한 첫째주 날짜부터 출력하되, gap이 7이 되면 줄을 바꾼다. -->
				<c:forEach begin="1" end="${lastDay}" varStatus="st">
		      	...
		      	<c:forEach var="nextDay" begin="${nextStartWeek}" end="7" varStatus="st">
		      		<td style="color:#ccc;font-size:0.8em">${nextMonth+1}-${st.count}</td>
		      	</c:forEach>
			</tr>
		</table>
	</div>
